const Accordion = ({ preTitle, title, children }: { preTitle: string, title: string, children: any }) => {
    return (
        <div className="mb-8 w-full rounded-lg bg-white p-4 shadow-[0px_20px_95px_0px_rgba(201,203,204,0.30)] dark:bg-dark-2 dark:shadow-[0px_20px_95px_0px_rgba(0,0,0,0.30)] sm:p-8 lg:px-6 xl:px-8">
            <button
                className={`faq-btn block w-full text-left outline-none`}
            >
                <div className="accordion-action float-right mr-5 flex h-10 w-full max-w-[40px] items-center justify-center rounded-lg bg-primary/5 text-primary dark:bg-white/5">
                    <svg
                        className={`accordion-icon fill-primary stroke-primary duration-200 ease-in-out transition-transform`}
                        width="17"
                        height="10"
                        viewBox="0 0 17 10"
            ...
            <div className={`accordion-content overflow-hidden max-h-0 duration-200 ease-in-out`}>
                {children}
            </div>
        </div>
    );
